...
/*   Updated: 2022/07/12 17:15:08 by yukim            ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */

#include "../include/minishell.h"

static void	rl_heredoc(t_data *data, char *delimiter, t_heredoc_fd *heredoc_fd, int d_quote);
static int	is_heredoc_env(char *value);
static char	*replace_env_in_heredoc(t_data *data, char *origin);
static	int	get_len_with_envvalue_in_heredoc(t_data *data, char *origin);
...
void	exec_heredoc(t_data *data, int i, int *idx)
{
	...
	{
		while (data->lexer.pptokens[i + 1][j])
		{
			if (is_quote(data->lexer.pptokens[i + 1][j]) != FALSE)
				delimiter_quote_flag = TRUE;
			j++;
		}
		if (delimiter_quote_flag)
			rm_argument_quote(data, i + 1);
		delimiter = data->lexer.pptokens[i + 1];
		rl_heredoc(data, delimiter, &data->heredoc_fd[*idx], delimiter_quote_flag);
		(*idx)++;
	}
}
...
static void	rl_heredoc(t_data *data, char *delimiter, t_heredoc_fd *heredoc_fd, int d_quote)
{
	...
	while (1)
	{
		...
		{
			env_str = replace_env_in_heredoc(data, input_str);
			free(input_str);
			input_str = env_str;
		}
...
static char	*replace_env_in_heredoc(t_data *data, char *origin)
{
	...
	j = 0;
	value = NULL;
	all_len = get_len_with_envvalue_in_heredoc(data, origin);
	ret = ft_calloc(all_len + 1, sizeof(char));
	if (!ret)
		ft_error("replace_env_in_heredoc : allocation failed\n");
	heredoc_replace_env_loop(data, origin, ret, value);
	return (ret);
}
...
static	int	get_len_with_envvalue_in_heredoc(t_data *data, char *origin)
{
	...
	while (origin[i])
	{
		...
		if (origin[i] == '$' && origin[i + 1] == '?')
			...
			len_loop_in_heredoc(data, origin, value, &i, &ret_len);
		else
		{
			i++;
			ret_len++;
