...
void	tree_reds(t_astnode *ast_node, int index, t_data *data)
{
	...
	if (type[index] == T_REDIRECTION)
	{
		...
		reds_leftchild->pleftchild = insert_leftchildnode_ast(ast_node, A_REDIRECTION_TYPE);
		reds_leftchild->prightchild = insert_rightchildnode_ast(ast_node, A_FILENAME);
